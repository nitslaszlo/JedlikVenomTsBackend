...
export class CsudijoRoutes {
  ...
  public routes(app: any): void {
    ...
    app.route("/csudijo/count").get((req: Request, res: Response, next: NextFunction) => {
      console.log(`Request type ${req.method} from: ${req.originalUrl} time: ${new Date().toLocaleTimeString()}`);
      next();
    }, this.csudijoController.getNumberOfFoods);

    app.route("/csudijo/page/:page/perPage/:perPage").get((req: Request, res: Response, next: NextFunction) => {
      console.log(`Request type ${req.method} from: ${req.originalUrl} time: ${new Date().toLocaleTimeString()}`);
      next();
